  ...
  const fetchOwner = async () => {
    try {
      ...
      if (data.success) {
        ...
        if (ownerData.picture) {
          setImagePreview(ownerData.picture)
        }
      }
    } catch (error) {
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-800">Edit Owner</h1>
            <p className="text-gray-600 mt-1">Update the owner's details.</p>
          </div>
        </div>
        <button
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="space-y-2">
              <label className="block text-sm font-medium text-gray-700">Owner's Picture</label>
              <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                <div className="space-y-1 text-center">
                  {imagePreview ? (
                    <img src={imagePreview} alt="Owner preview" className="mx-auto h-32 w-32 object-cover rounded-full" />
                  ) : (
